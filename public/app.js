	...
	const contractAddress = '0xE3c51112f181767aD97a76c8b182A1BdAed78475';
	const contractABI = [
		{
			"inputs": [],
		...
		{
			"inputs": [],
			"name": "getAvailableBlood",
			"outputs": [
				{
					"internalType": "uint256[4]",
					"name": "availableBloodResults",
					"type": "uint256[4]"
				}
			],
			"stateMutability": "view",
	...
	// Function to display blood requests on the admin dashboard
	async function displayBloodRequests() {
		try {
			const { registeredPatients } = await bloodBankContract.methods.getRegisteredUsers().call();

			const bloodRequestsList = document.getElementById('bloodRequestsList');

			// Clear existing content in the list
			while (bloodRequestsList.firstChild) {
				bloodRequestsList.removeChild(bloodRequestsList.firstChild);
			}

			// Enum representing blood types
			const BloodType = {
				A: 0,
				B: 1,
				AB: 2,
			...
			// Iterate through registered patients and their blood types
			for (const patientAddress of registeredPatients) {
				for (const bloodType in BloodType) {
					if (BloodType.hasOwnProperty(bloodType)) {
						const requests = await bloodBankContract.methods.getPatientRequests(patientAddress, BloodType[bloodType]).call();

						// Display blood requests in the list
						requests.forEach((request) => {
							const listItem = document.createElement('li');
							listItem.textContent = `Blood request from patient at ${patientAddress}. Blood Type: ${bloodType}, Amount: ${request.amount}`;
							bloodRequestsList.appendChild(listItem);
						});
					}
				}
			...
			console.error('Error during displaying blood requests:', error.message);
		}
	}
...
//0x407b486ad62B4e69D72A87B54D866DbeDe3540fA
//0x407b486ad62B4e69D72A87B54D866DbeDe3540fA  
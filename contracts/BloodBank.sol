        ...
        contract BloodBank {
            ...
            mapping(BloodType => uint) public availableBlood;
            ...
            bloodInventory[bloodType] += amount;  // Increment the total blood inventory for the blood type
        ...
        function getPatientRequests(address patientAddress, BloodType bloodType) public view returns (Request[] memory) {
            ...
            // Function to calculate available blood for each blood type
            function getAvailableBlood() public view returns (uint[4] memory availableBloodResults) {
            // Initialize array to store available blood for each blood type (A, B, AB, O)
            for (uint8 i = 0; i < 4; i++) {
                BloodType bloodType = BloodType(i);
                
                // Calculate available blood for each type
                uint donated = bloodInventory[bloodType];
                uint requested = getPendingRequestsByType(bloodType);
                
                // Subtract requested from donated to get available
                if (donated >= requested) {
                    availableBloodResults[i] = donated - requested;
                } else {
                    availableBloodResults[i] = 0; // Avoid negative values
                }
            }

            return availableBloodResults;
        }

            // Function to get pending requests for a specific blood type
            function getPendingRequestsByType(BloodType bloodType) internal view returns (uint pending) {
            uint totalPending;
            address[] memory patientAddress = getRegisteredPatients();

            // Loop through all patients and sum up pending requests for the specified blood type
            for (uint i = 0; i < patientAddress.length; i++) {
                Request[] memory requests = patientRequests[patientAddress[i]][bloodType];

                for (uint j = 0; j < requests.length; j++) {
                    if (!requests[j].isResponded) {
                        totalPending += requests[j].amount;
                    }
                }
            }
            ...
            function getRegisteredPatients() internal view returns (address[] memory) {
